...
--    from the Models and Brands tables.
    ...
    SELECT brand_name, models.name, headquarters FROM Models JOIN Brands ON brands.name = brand_name WHERE brand_name = 'Ford' AND models.name = 'Mustang';
...
-- 8. Select rows 15-25 of the DB in alphabetical order by model name.
    ...
    SELECT * FROM Brands JOIN Models ON brands.name = brand_name ORDER BY brands.name LIMIT 10 OFFSET 14;
...
--    the brand is not in the Brands table.)
    ...
    SELECT brand_name, models.name, founded FROM Models LEFT JOIN Brands ON brands.name = brand_name WHERE year = 1960;
...
--   IIHS Safety Award    2015      # get the ``id`` of the 2015 Subaru Outback
    ...
    INSERT INTO Awards VALUES ('IIHS Safety Award',2015, (SELECT id FROM Models WHERE brand_name = 'Chevrolet' AND name = 'Malibu')),('IIHS Safety Award',2015,
        (SELECT id FROM Models WHERE brand_name = 'Subaru' AND name = 'Outback'));
    
-- 5. Using a subquery, select only the *name* of any model whose 
-- year is the same year that *any* brand was founded.

    SELECT models.name FROM Models JOIN Brands WHERE year IN (SELECT founded FROM Brands GROUP BY founded) GROUP BY models.name;
